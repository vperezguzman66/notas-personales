...
- **Dashboard web local** (`npm run web`, `http://127.0.0.1:4100`, solo local — nunca expuesto a la red): pestañas de propuestas pendientes (aprobar/rechazar con botones), decididas, corpus indexado, y búsqueda semántica. Requirió un refactor previo (lógica de retrieval y propuestas movida a `search.js`/`proposals-store.js`, reutilizada por CLI y web). Probado en el navegador de punta a punta, incluyendo aprobar una propuesta real con un click.
- **Chat conversacional standalone (2026-07-16, probado de punta a punta el 2026-07-17):** un segundo modo de uso, además del skill de Claude Code — un agente propio que llama directo a la Messages API de Anthropic con tool use, en vez de seguir el guion fijo del skill. Vive en `src/chat-agent.js` (loop de tool-use), `src/chat-tools.js` (las tools), `src/chat-store.js` (memoria de conversación en SQLite), `src/chat-cli.js` (REPL de terminal) y `src/market-data.js` (cliente de Twelve Data). Se invoca con `npm run chat` o `POST /api/chat` en el dashboard.
  - **Prueba real (2026-07-17):** `search_research` trajo y citó bien los chunks del corpus (riesgos de NVDA, competencia AMD, financiamiento circular); memoria entre turnos confirmada (la segunda pregunta usó el contexto de la primera sin repetir retrieval); `propose_order` guardó una propuesta `pending` válida y bien razonada para NVDA; `get_market_data` trajo cotización + RSI/EMA reales de Twelve Data, confirmando que la forma de la respuesta asumida en el código era correcta. Bug real encontrado y arreglado: `chat-cli.js` explotaba si el input llegaba por pipe/EOF mientras la respuesta seguía en vuelo (`rl.prompt()` sobre una interfaz ya cerrada).
  - Esto era exactamente lo que antes bloqueaba tener un botón de "analizar" en el dashboard (ver más abajo, "Cuentas/servicios externos"): requería una API key propia de Anthropic, y el MCP de TradingView no es alcanzable desde un servidor web. Ahora se resuelve con una `ANTHROPIC_API_KEY` propia y **Twelve Data** (REST) como fuente de datos vivos en vez de TradingView MCP — que sigue siendo exclusivo del skill dentro de Claude Code.
  - Tools expuestas al modelo: `search_research`, `get_market_data`, `propose_order`. **`decide_proposal` queda deliberadamente fuera** — la aprobación de una propuesta nunca puede quedar en manos de que el modelo interprete una frase ambigua del chat ("dale, apruébala"); sigue siendo una acción humana explícita y separada (dashboard o `npm run decide`), igual que con el skill.
...
- **Voyage AI** — cuenta + API key, solo para embeddings.
- **Skill `/analizar-inversion` (Claude Code):** no necesita API key propia de Anthropic — el razonamiento lo hace la sesión de Claude Code misma, no un script separado orquestando llamadas a la API.
- **Chat standalone (nuevo, 2026-07-16):** sí necesita su propia **`ANTHROPIC_API_KEY`** (llama a la Messages API desde afuera de Claude Code) y una cuenta de **Twelve Data** (cotización + indicadores, en vez del MCP de TradingView que ese modo no puede alcanzar).
